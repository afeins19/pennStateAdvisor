...
def clean_text_for_major(major_data_dict):
    ...
    illegal_patterns = {
        r"\xa0": " ",
        r"\s+": " ",
    }  # Non-breaking spaces and extra spaces
...
def get_baccalaureate_degree_links():
    ...
    degree_links = [
        urljoin(BASE_URL, a["href"]) for a in soup.select(".sitemap_visual li a")
    ]
    return degree_links
...
def extract_additional_courses(major_url):
    ...
    for row in soup.select(".sc_courselist tr.odd, .sc_courselist tr.even"):
        ...
        if "orclass" in row.get("class", []):
            ...
            course_list[-1]["equivalent_course"] = course_list[-2][
                "course_code"
            ]  # make the prev course iterated over the equivalent one
...
def extract_selectable_courses(major_url):
    ...
    for row in soup.find_all(
        "tr"
    ):  # Loop through table rows to capture both headers and courses
        columns = row.find_all("td")

        if (
            len(columns) == 2
        ):  # Selection header row (e.g., "Select one of the following")

            header_text = columns[0].get_text(strip=True).lower()
            required_credits = columns[1].get_text(
                strip=True
            )  # Credits in second column
            ...
            course_url = (
                urljoin(BASE_URL, course_link["href"]) if course_link else "N/A"
            )
...
def generate_major_requirements(major_url):
    ...
    )  # TODO make soup a parameter to be passed to all 'extract_...' functions

    major_title = soup.find(class_="page-title").get_text(
        strip=True
    )  # Accounting, B.S. (Abington)
    major_code = soup.find(id="program-code").get_text(strip=True)
    major_code = major_code.replace("Program Code: ", "").split("_")[0]  # ACCAB_BS -> ACCAB
...
def save_dataframe_to_csv(data, filename="major_requirements.csv"):
...
if __name__ == "__main__":
    ...
    for link in links:
        major_data.append(generate_major_requirements(link))
    
    #save_dataframe_to_csv(major_data)
